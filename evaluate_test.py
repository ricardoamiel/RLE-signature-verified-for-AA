# evaluate_test.py (solo reemplaza el archivo)
import json
import argparse
...
def load_thresholds(calib_path, fallback=0.85):
    # Soporta calibration_maxstp.json con {thresholds:{p85:{...}, p80:{...}}}
    thr_dict = {"p85": fallback, "p80": 0.80}
    meta = None
    ...
    if p and p.exists():
        try:
            ...
            if "thresholds" in d:
                for k in ("p85", "p80"):
                    if k in d["thresholds"]:
                        thr_dict[k] = float(d["thresholds"][k]["threshold"])
            elif "threshold" in d:
                thr_dict["p85"] = float(d["threshold"])
            meta = d
        except Exception:
            pass
    return thr_dict, meta
...
def main():
    ...
    out_dir = Path(args.out_dir); (out_dir / "test_plots").mkdir(parents=True, exist_ok=True)
    device = "cpu" if (args.cpu or not torch.cuda.is_available()) else "cuda"

    test_loader, _ = build_eval_loader(
    ...
    thr_dict, calib_meta = load_thresholds(args.calib, fallback=args.default_threshold)
    y_true, y_prob = collect_scores(model, test_loader, device)
    ...
    # Métricas a p85 y p80
    conf_p85 = confusion_at_threshold(y_true, y_prob, thr_dict["p85"])
    conf_p80 = confusion_at_threshold(y_true, y_prob, thr_dict["p80"])
    ...
    pd.DataFrame({"y_true": y_true.astype(int),"y_prob": y_prob}).to_csv(pred_csv, index=False)

    summary = {
        "timestamp": datetime.utcnow().isoformat(),
        "checkpoint": str(args.ckpt),
        "calibration_file": str(args.calib) if args.calib else None,
        "thresholds_used": thr_dict,
        "test_auc": float(auc_val),
        "test_eer": float(eer_val),
        "metrics_at_p85": conf_p85,
        "metrics_at_p80": conf_p80,
        "pairs_per_writer": args.pairs_per_writer,
        "batch_size": args.batch_size,
        "img_size": args.img_size,
    ...
    print(f"p85 thr={thr_dict['p85']:.6f} | STP={conf_p85['stp_proxy']:.3f} | P={conf_p85['precision']:.3f} | R={conf_p85['recall']:.3f}")
    print(f"p80 thr={thr_dict['p80']:.6f} | STP={conf_p80['stp_proxy']:.3f} | P={conf_p80['precision']:.3f} | R={conf_p80['recall']:.3f}")
